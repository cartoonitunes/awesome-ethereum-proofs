...
const FILE=path.join(__dirname,'soljson-v0.1.3.js');
...
async function main(){
 ...
 const solc=require(FILE);const out=JSON.parse(solc.cwrap('compileJSON','string',['string','number'])(SRC,0));
 if(out.errors){console.log('ERRORS',out.errors);process.exit(1);}
 const bin=out.contracts['DepositWalletV2'].bytecode.toLowerCase();const ok=bin.includes(TARGET);
