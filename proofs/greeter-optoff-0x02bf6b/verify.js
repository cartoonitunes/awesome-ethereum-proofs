#!/usr/bin/env node
// Reproducible verification for 0x02bf6B982C2aFDB7c1209E3D2f283162Ae3842f6
// Downloads soljson-v0.2.0+commit.4dc2445e, compiles greeter.sol (optimizer OFF), checks bytecode byte-for-byte.
...
(async()=>{
  ...
  let raw;
  if(typeof m._compileJSONMulti==='function'){raw=m.cwrap('compileJSONMulti','string',['string','number'])(JSON.stringify({sources:{'greeter.sol':src}}),0);}
  ...
  const c=out.contracts['greeter']||out.contracts['greeter.sol:greeter'];
  ...
  const rOk=rt===tRt;
  console.log('compiler        : soljson-v0.2.0+commit.4dc2445e (optimizer OFF)');
