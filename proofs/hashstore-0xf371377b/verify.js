...
const URL='https://binaries.soliditylang.org/bin/soljson-v0.4.18+commit.9cf6e910.js';
const P=path.join(__dirname,'soljson-v0.4.18+commit.9cf6e910.js');
...
(async()=>{
  await dl(URL,P);
  const m=require(P);
  const src=fs.readFileSync(path.join(__dirname,'HashStore.sol'),'utf8');
  const raw=m.cwrap('compileStandard','string',['string','number'])(JSON.stringify({language:'Solidity',sources:{'HashStore.sol':{content:src}},settings:{optimizer:{enabled:true,runs:200},outputSelection:{'*':{'*':['evm.deployedBytecode.object']}}}}),0);
  const rt=JSON.parse(raw).contracts['HashStore.sol']['HashStore'].evm.deployedBytecode.object.toLowerCase();
  ...
  console.log('compiler: soljson-v0.4.18+commit.9cf6e910 (optimizer ON, runs=200)');
