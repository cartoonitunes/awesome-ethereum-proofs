#!/usr/bin/env node
// Reproducible verification for 0x1C606eE0c43f60166AB497B33548f7E4E909d447
// Downloads soljson-v0.1.3+commit.028f561d, compiles subcurrency.sol (optimizer ON), checks bytecode byte-for-byte.
...
(async()=>{
  ...
  const src=fs.readFileSync(path.join(__dirname,'subcurrency.sol'),'utf8');
  let raw;
  if(typeof m._compileJSONMulti==='function'){raw=m.cwrap('compileJSONMulti','string',['string','number'])(JSON.stringify({sources:{'subcurrency.sol':src}}),1);}
  ...
  const c=out.contracts['Subcurrency']||out.contracts['subcurrency.sol:Subcurrency'];
