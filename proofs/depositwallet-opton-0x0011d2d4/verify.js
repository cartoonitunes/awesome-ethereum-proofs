#!/usr/bin/env node
// Reproducible RUNTIME + CREATION verification for 0x0011d2D4f8d91D955b7FedBE04D8a0600ea657F2
// Downloads soljson-v0.1.3+commit.028f561d, compiles depositwallet.sol (optimizer ON), and checks
// the compiled runtime and creation bytecode each equal the on-chain bytecode, byte for byte.
//
//   node verify.js
//
...
(async () => {
  ...
  if (typeof m._compileJSONMulti === 'function') {
    ...
    raw = f(JSON.stringify({ sources: { 'depositwallet.sol': source } }), 1); // optimizer ON
  } else {
  ...
  const c = out.contracts['DepositWallet'] || out.contracts['depositwallet.sol:DepositWallet'];
