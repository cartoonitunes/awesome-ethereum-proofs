#!/usr/bin/env node
// Reproducible RUNTIME verification for 0x0a5763e438C8f4156Ea4347014852df39a77b3A1
// Downloads soljson-v0.1.3+commit.028f561d, compiles timelock.sol (optimizer ON), and checks the
// compiled Timelock runtime equals the on-chain runtime, byte for byte.
//   node verify.js   -> exit 0 = exact runtime match.
'use strict';
const fs = require('fs');
...
(async () => {
  ...
  const source = fs.readFileSync(path.join(__dirname, 'timelock.sol'), 'utf8');
  let raw;
  if (typeof m._compileJSONMulti === 'function') {
    const f = m.cwrap('compileJSONMulti', 'string', ['string', 'number']);
    raw = f(JSON.stringify({ sources: { 'timelock.sol': source } }), 1); // optimizer ON
  } else {
  ...
  const c = out.contracts['Timelock'] || out.contracts['timelock.sol:Timelock'];
  ...
  console.log('runtime match   :', match, '(' + target.length / 2 + ' bytes, sha256 ' + sha + ')');
