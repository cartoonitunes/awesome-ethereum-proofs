#!/usr/bin/env node
// Verify Cyrus Adkisson's mortal contract reproduces the on-chain bytecode byte-for-byte.
// Two byte-identical deployments: 0xa5e57510… and 0x63989aae…. Self-destructed on-chain; EH preserves the historical creation bytecode.
// Usage: npm i solc@0.4.26 && node verify.js
const fs = require('fs'), https = require('https'), path = require('path');
const VER = 'soljson-v0.1.5+commit.23865e39.js';
const OPT = 1, CNAME = 'mortal', SRC = 'mortal.sol';
...
findSoljson(soljsonPath => {
  ...
  let ok = true;
  for (const [mode,file,field] of [['creation','target_creation.txt','bytecode']]) {
